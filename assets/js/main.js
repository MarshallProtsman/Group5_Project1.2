$(document).ready(function () {
  ...
  let loaderText = '';
  let eventList = []; //empty event list array to store into Firebase
  ...
  let callAPIGeo = function () {
    ...
    $.ajax({
      ...
      //stores the city search into firebase
      database.ref().set({
        city: city
      });
    });
  ...
  let callAPIEvents = function () { // Begin ajax call for events
    ...
    $.ajax({
      ...
      for (i = 0; i < response._embedded.events.length; i++) {
        ...
        eventList.push(response._embedded.events[i].name); //pushes each event into Event name array
        let eventItem = $('<div>');
  ...
  console.log(snapshot.val().eventList);
  $("#recentEventList").text(snapshot.val().eventList);
